...
class SolanaColdWalletCLI:
    ...
    def _check_usb_for_wallet(self, mount_point: str) -> tuple:
        ...
        dilithium_pub = wallet_dir / "dilithium_pub.bin"
        dilithium_sec = wallet_dir / "dilithium_sec.bin"

        if (
            pubkey_path.exists()
            and dilithium_pub.exists()
            and dilithium_sec.exists()
        ):
    ...
    def _usb_detected_menu(self, devices):
        ...
        options = [
            ...
            "1. Mount USB (Check for existing wallet)",
            "2. Network Status",
            "0. Exit"
        ]
