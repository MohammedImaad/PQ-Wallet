...
class TokenSecurityPanel(Static):
    ...
    def compose(self) -> ComposeResult:
        yield Static(" Transaction Logs", classes="panel-header")
        yield ScrollableContainer(
    ...
    def refresh_security(self) -> None:
        ...
        if self.security_data and "live_logs" in self.security_data:
            content.update(self.security_data["live_logs"])
            return
...
class ColdstarLiveWallet(App):
    ...
    def _do_send(self, to_addr: str, amount: float, password: str, review: Static) -> None:
        """Send transaction + PQC authorize + rotate wallet + migrate funds"""
        ...
        try:
            ...
            def log_step(message):
                state_log.append(message)

                security_panel = self.query_one(TokenSecurityPanel)
            ...
            # INITIALIZE
            # =========================================================
            old_wallet_address = self.current_public_key

            log_step("[bold cyan]COLDSTAR TRANSACTION PIPELINE[/bold cyan]")
            log_step("")

            # =========================================================
            # STEP 1: BLOCKHASH
            # =========================================================
            log_step("[cyan][INIT][/cyan] Fetching blockhash")

            blockhash_result = self.network.get_latest_blockhash()

            if not blockhash_result:
                log_step("[red]✗ Failed to fetch blockhash[/red]")
                return

            blockhash, _ = blockhash_result

            log_step("[green]✓ Blockhash fetched[/green]")

            # =========================================================
            # STEP 2: BUILD TX
            # =========================================================
            log_step("[cyan][BUILD][/cyan] Building transaction")
            ...
            if not unsigned_tx:
                log_step("[red]✗ Failed to build transaction[/red]")
                return

            log_step("[green]✓ Transaction built[/green]")

            # =========================================================
            # STEP 3: LOAD WALLET
            # =========================================================
            log_step("[cyan][LOAD][/cyan] Loading secure wallet")
            ...
            if not encrypted_container:
                log_step("[red]✗ Failed to load wallet[/red]")
                return

            log_step("[green]✓ Wallet decrypted[/green]")

            # =========================================================
            # STEP 4: SIGN TX
            # =========================================================
            log_step("[cyan][SIGN][/cyan] Signing transaction")
            ...
            if not signed_tx:
                log_step("[red]✗ Signing failed[/red]")
                return

            log_step("[green]✓ Transaction signed[/green]")

            # =========================================================
            # STEP 5: PQC AUTHORIZATION
            # =========================================================
            log_step("[cyan][PQC][/cyan] Creating rotation authorization")

            rotation_message = f"""
    old_wallet:{old_wallet_address}
    destination_wallet:{to_addr}
    amount:{amount}
    timestamp:{time.time()}
    """

            signature = self.wallet_manager.sign_rotation_message(
                rotation_message.encode()
            )

            log_step("[green][PQC][/green] Rotation signed")

            self.wallet_manager.verify_rotation_signature(
                rotation_message.encode(),
                signature
            )

            log_step("[green][PQC][/green] Signature verified")

            # =========================================================
            # STEP 6: BROADCAST
            # =========================================================
            log_step("[cyan][BROADCAST][/cyan] Broadcasting transaction")
            ...
            if not signature:
                log_step("[red]✗ Transaction rejected[/red]")
                return

            sig_short = f"{signature[:8]}...{signature[-8:]}"

            log_step(f"[green]✓ Broadcast success[/green] {sig_short}")

            # =========================================================
            # WAIT FINALIZATION
            # =========================================================
            log_step("[yellow][VERIFY][/yellow] Waiting for finalization")

            time.sleep(20)

            log_step("[green]✓ Finalized[/green]")

            # =========================================================
            # STEP 7: ARCHIVE OLD WALLET
            # =========================================================
            log_step("[cyan][ROTATE][/cyan] Archiving old wallet")

            archive_dir = Path(self.usb_manager.mount_point) / "wallet_archive"
            ...
            shutil.copy(
                keypair_path,
                archive_dir / f"keypair_{timestamp}.json"
            )

            shutil.copy(
                pubkey_path,
                archive_dir / f"pubkey_{timestamp}.txt"
            )

            log_step("[green]✓ Wallet archived[/green]")

            # =========================================================
            # STEP 8: GENERATE NEW WALLET
            # =========================================================
            log_step("[cyan][ROTATE][/cyan] Generating fresh wallet")

            self.wallet_manager.clear_memory()

            gc.collect()

            self.wallet_manager.generate_keypair()

            log_step("[green]✓ Fresh wallet generated[/green]")

            # =========================================================
            # STEP 9: SAVE NEW WALLET
            # =========================================================
            log_step("[cyan][ROTATE][/cyan] Encrypting new wallet")

            encrypted_data = SecureWalletHandler.encrypt_keypair(
            ...
            with open(pubkey_path, "w") as f:
                f.write(str(self.wallet_manager.keypair.pubkey()))

            new_wallet_address = str(
                self.wallet_manager.keypair.pubkey()
            )

            log_step("[green]✓ New wallet encrypted[/green]")

            # =========================================================
            # STEP 10: CHECK BALANCE
            # =========================================================
            log_step("[cyan][SWEEP][/cyan] Checking remaining balance")

            updated_balance = self.network.get_balance(
                old_wallet_address
            )
            ...
            log_step(
                f"[green]✓ Remaining balance:[/green] "
                f"{updated_balance:.6f} SOL"
            )
            ...
            if updated_balance > 0.01:
                ...
                sweep_amount = updated_balance - 0.005

                log_step("[green][PQC][/green] Sweep authorized")

                log_step(
                    f"[cyan][SWEEP][/cyan] Sweeping "
                    f"{sweep_amount:.6f} SOL"
                )

                sweep_blockhash_result = self.network.get_latest_blockhash()

                if not sweep_blockhash_result:
                    log_step("[red]✗ Sweep blockhash failed[/red]")
                    return

                sweep_blockhash, _ = sweep_blockhash_result

                sweep_unsigned = self.transaction_manager.create_transfer_transaction(
                    old_wallet_address,
                    new_wallet_address,
                    sweep_amount,
                    sweep_blockhash
                ...
                if not sweep_unsigned:
                    log_step("[red]✗ Sweep tx build failed[/red]")
                    return

                log_step("[green]✓ Sweep transaction built[/green]")

                sweep_signed = self.transaction_manager.sign_transaction_secure(
                    sweep_unsigned,
                    encrypted_container,
                    password
                )
                ...
                if not sweep_signed:
                    log_step("[red]✗ Sweep signing failed[/red]")
                    return

                log_step("[green]✓ Sweep transaction signed[/green]")

                sweep_base64 = base64.b64encode(
                    sweep_signed
                ).decode("utf-8")

                sweep_signature = self.network.send_transaction(
                    sweep_base64
                )

                if not sweep_signature:
                    log_step("[red]✗ Sweep broadcast failed[/red]")
                    return
                ...
                log_step(
                    f"[green]✓ Sweep successful[/green] "
                    f"{sweep_short}"
                )

            else:
                log_step("[yellow]→ No remaining funds to migrate[/yellow]")

            # =========================================================
            # STEP 12: SWITCH ACTIVE WALLET
            # =========================================================
            log_step("[cyan][ROTATE][/cyan] Switching active wallet")

            self.current_public_key = new_wallet_address

            log_step("[green]✓ Active wallet updated[/green]")

            # =========================================================
            # STEP 13: CLEAR MEMORY
            # =========================================================
            log_step("[cyan][SECURITY][/cyan] Clearing secure memory")

            self.wallet_manager.keypair = None

            gc.collect()

            log_step("[green]✓ Secure memory cleared[/green]")

            # =========================================================
            # COMPLETE
            # =========================================================
            log_step("")
            log_step("[bold green]✓ WALLET ROTATION COMPLETE[/bold green]")
            log_step("")
            log_step(f"[cyan]New Wallet:[/cyan]")
            log_step(new_wallet_address)
            ...
            log_step(f"[red]✗ ERROR:[/red] {str(e)[:120]}")
