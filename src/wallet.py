...
class WalletManager:
    def __init__(self, wallet_dir: str = None):
        ...
        self.dilithium_pub_path: Optional[Path] = None
        self.dilithium_sec_path: Optional[Path] = None
    ...
    def set_wallet_directory(self, path: str):
        ...
        self.dilithium_pub_path = self.wallet_dir / "dilithium_pub.bin"
        self.dilithium_sec_path = self.wallet_dir / "dilithium_sec.bin"
    ...
    def save_keypair(self, path: str = None) -> bool:
        ...
        try:
            ...
            dilithium_pub, dilithium_sec = self.generate_pqc_keypair()

            # Save PQC public key
            with open(self.dilithium_pub_path, "wb") as f:
                f.write(dilithium_pub)

            # Save PQC private key
            with open(self.dilithium_sec_path, "wb") as f:
                f.write(dilithium_sec)

            os.chmod(self.dilithium_sec_path, 0o600)

            print_success("✓ Dilithium PQC identity generated")
    ...
    def sign_rotation_message(self, message: bytes):
        if self.keypair_path is None:
            print_error("Wallet path not initialized")
            return None

        dilithium_sec_path = Path(self.keypair_path).parent / "dilithium_sec.bin"

        with open(dilithium_sec_path, "rb") as f:
            secret_key = bytearray(f.read())
    ...
    def verify_rotation_signature(
        ...
        dilithium_pub_path = Path(self.keypair_path).parent / "dilithium_pub.bin"
